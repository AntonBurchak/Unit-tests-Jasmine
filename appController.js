class AppController {
    ...
    renderAddPizzaButtonInOrder() {
        const addButtonTemplate = `<button class="button is-primary full-width mb-20 add-pizza" onclick="appController.addPizzaForm()">Add pizza</button>`;
        document.querySelector('.order').insertAdjacentHTML('beforeend', addButtonTemplate);
    }

    renderTotalPriceInOrder() {
        const totalPriceTemplate = `<div class="is-size-2 totalP">Total Price: ${appController.order.totalPrice}$</div>`;
